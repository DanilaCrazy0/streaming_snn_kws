"""Recurrent GSU-branch streaming KWS with sum-threshold fusion.

Thesis architecture «RNN»: each frequency branch is one shared GSU cell
unrolled R times per STFT frame. This is the best published model.

Best overall result in the thesis:
    k=2, R=3, H=256, n_fft=512, hop=64, preset p3_default (4 branches)
    val 0.9312 / test 0.9268, 1_718_051 parameters
...
# RecurrentSpikeHead: first micro-cell uses full GSUCell with h_prev from prior frame.
# Based on fast_learning_rnn.py; original left unchanged.
...
def select_gsc_rows(config: SpikeFusionConfig, split: str) -> list[dict]:
    return [row for row in build_gsc_frame(config.to_gsc_audio_config()) if row["split"] == split]
...
def make_gsc_dataloaders(
    ...
    train_rows = select_gsc_rows(config, split="train")
    val_rows = select_gsc_rows(config, split="val")
    test_rows = select_gsc_rows(config, split="test")
...
class RecurrentSpikeHead(nn.Module):
    ...
    def forward_step(
        ...
        dynamics = {
            "projected_input": proj.detach(),
            "micro_spikes": torch.stack(cell_spikes, dim=0).detach(),
            "layer_spikes": torch.stack(cell_spikes, dim=1).detach(),
            "layer_potentials": torch.stack(cell_potentials, dim=1).detach(),
        }
        return branch_spikes, new_state, dynamics
...
class StreamingSpikeFusionClassifier(nn.Module):
    def __init__(self, config: SpikeFusionConfig, k: int):
        super().__init__()
        self.config = config
        self.k = int(k)
        self.recurrency = int(config.recurrency)
        ...
        for name in self.branch_names:
            ...
            self.branch_heads[name] = RecurrentSpikeHead(
    ...
    def forward(self, frames: torch.Tensor, return_dynamics: bool = False) -> dict:
        ...
        branch_states: dict[str, Optional[MemoryState]] = {name: None for name in self.branch_names}
...
def default_spike_fusion_checkpoint_path(k: int, recurrency: int) -> Path:
    return Path("notebooks/artifacts/flow_spike_fusion") / f"best_spike_fusion_k{k}_r{recurrency}.pkl"
...
def save_streaming_spike_fusion_checkpoint(
    ...
    payload = {
        ...
        "k": int(model.k),
        "recurrency": int(model.recurrency),
...
def fit_streaming_spike_fusion(
    ...
    lr_scheduler: str = "gated_plateau",
    warmup_epochs: int = 12,
    lr_gate_accuracy: float = 0.80,
    plateau_factor: float = 0.7,
    plateau_patience: int = 5,
    plateau_threshold: float = 1e-3,
    plateau_threshold_mode: str = "rel",
    ...
    device = resolve_device(device)
    recurrency = int(config.recurrency)
    checkpoint_path = (
        default_spike_fusion_checkpoint_path(k, recurrency)
        if checkpoint_path is None
        else Path(checkpoint_path)
    )
    model = StreamingSpikeFusionClassifier(config=config, k=k).to(device)
    ...
    if lr_scheduler_name == "cosine":
        ...
        raise ValueError(
            f"unknown lr_scheduler '{lr_scheduler}' (expected gated_plateau, cosine or none)."
        )
    ...
    for epoch_idx in range(epochs):
        ...
        epoch_row = {
            ...
            "lr_gate_reached": bool(gated_scheduler.gate_reached) if gated_scheduler is not None else False,
        ...
        if verbose:
            print(
                f"[k={k}, recurrency={recurrency}] epoch {epoch_idx + 1}/{epochs} | "
                f"lr={current_lr:.2e} "
    ...
    if verbose:
        print(
            f"[k={k}, recurrency={recurrency}] final test @ best val epoch {best_epoch} | "
...
def run_single_experiment(
    *,
    ...
    # (all branch layers/micro-steps + fusion layers), mean/std over the
    # validation set. ``fit_streaming_spike_fusion`` returns the model with the
    # best-val weights already loaded.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train streaming spike-fusion KWS with sum-threshold pre-fusion, "
            "GSN fusion_head, and shared-weight RNN branch recurrence."
        )
    )
    parser.add_argument("--cache-dir", default=DEFAULT_DATASET_CACHE_DIR)
    parser.add_argument("--artifact-root", default="artifacts/fast_learning_rnn_sumfusion")
    ...
    parser.add_argument(
        ...
        "which spikes at 0 on the gated membrane.",
    )
    parser.add_argument(
        "--skip-spike-count",
        action="store_true",
    ...
    parser.add_argument(
        "--lr-scheduler",
        default="gated_plateau",
        choices=("gated_plateau", "cosine", "none"),
        help="Learning-rate schedule: gated_plateau (fixed LR until val gate, then "
        "ReduceLROnPlateau), cosine (warmup + cosine decay), or none.",
    )
    parser.add_argument(
        "--warmup-epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        "(used with --lr-scheduler gated_plateau).",
    )
    parser.add_argument(
        "--plateau-factor",
        type=float,

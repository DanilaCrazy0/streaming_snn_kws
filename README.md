...
**4 фиксированных сида (7, 10, 12, 80)** на конфиг — 72 запуска, 0 сбоев.
Фиксировано: \(H=256\), \(N_{\mathrm{fft}}=512\), hop \(=64\), пресет `p3_default`,
30 эпох, batch 500, RTX 4090. Спайки — среднее число спайков всех слоёв (ветви +
...
Выводы: RNN стабильна по \(R\) при **фиксированном** числе параметров (одна общая
ячейка), тогда как GSN-layers растут по параметрам и спайкам и деградируют с
...
те же 4 сида — 24 запуска, 0 сбоев. Для сравнения: те же конфиги при \(H=256\)
...
Точность заметно проседает при \(H<128\): компромисс «параметры/спайки vs
качество» разумеется до \(H{=}128\) (−1.5–2 п.п. val при 3.4–3.5× меньше
параметров и ~1.6× меньше спайков), а \(H{=}32\) практически непригоден.
